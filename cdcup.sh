...
if [ "$1" == 'init' ]; then
  ...
  if [ ! -d ./cdc ]; then
    printf "❌ Missing ./cdc runtime directory. Please run ./cdcup.sh up first.\n"
    exit 1
  fi
  cdc_real_path="$(cd ./cdc && pwd -P)"
  pipeline_real_path="$(cd "$(dirname "$2")" && pwd -P)/$(basename "$2")"
  printf "🚩 Submitting pipeline job...\n"
  if ! docker compose exec -T jobmanager bash -c 'test -x /opt/flink-cdc/bin/flink-cdc.sh' >/dev/null 2>&1; then
    printf "⚠️  /opt/flink-cdc missing in jobmanager, syncing runtime files...\n"
    docker compose exec -T jobmanager bash -c 'rm -rf /opt/flink-cdc && mkdir -p /opt'
    docker compose cp "${cdc_real_path}" jobmanager:/opt/flink-cdc
  fi
  docker compose cp "${pipeline_real_path}" jobmanager:/opt/flink-cdc/pipeline-definition.yaml
  ...
  docker compose exec -T jobmanager bash -c "$startup_script"
elif [ "$1" == 'flink' ]; then
